public class Clock {
    ...
    public Clock(int hour, int minute){
        if (hour < 0 || hour > 23) throw new IllegalArgumentException("hours must be between 0 and 23");
    ...
    public Clock(String s) throws IllegalArgumentException {
        if (s.length() != 5) throw new IllegalArgumentException("the format must be HH:MM");
        if (Character.compare(s.charAt(2), ':') != 0) throw new IllegalArgumentException("the format must be HH:MM");
        int hour, minute;
        try {
    ...
    public void toc(int delta) {
        if (delta < 0) throw new IllegalArgumentException("Δ must be greater than 0");
        m += delta;
        h += m / 60;
        h = h % 24;
        m = m % 60;
